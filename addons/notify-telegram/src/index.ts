/**
 * Telegram notifications - Aetheris notification module.
 *
...
export class TelegramNotifier implements NotificationChannel {
  readonly id = "notify-telegram";
  private readonly config: NotifyConfig;

  constructor(config: NotifyConfig) {
    if (!config.webhookUrl) throw new Error("notify-telegram: webhookUrl is required");
    this.config = config;
  }
  ...
  async send(message: NotificationMessage): Promise<void> {
    ...
    if (!response.ok) {
      throw new Error(`notify-telegram: HTTP ${response.status} while sending notification`);
    }
  }
  ...
  async test(): Promise<void> {
    await this.send({ title: "Test notification", body: "Aetheris notify-telegram is configured correctly." });
  }
}
