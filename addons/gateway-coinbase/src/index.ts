/**
 * Coinbase Commerce - Aetheris payment gateway module.
 *
...
export class CoinbaseGateway implements PaymentGateway {
  readonly id = "gateway-coinbase";
  private readonly config: GatewayConfig;

  constructor(config: GatewayConfig) {
    if (!config.apiKey) throw new Error("gateway-coinbase: apiKey is required");
    this.config = config;
  }
  ...
  private async request<T>(path: string, init: RequestInit = {}): Promise<T> {
    ...
    if (!response.ok) {
      throw new Error(`gateway-coinbase: HTTP ${response.status} on ${path}`);
    }
  ...
  async createCheckout(amountCents: number, currency: string, metadata: Record<string, string>): Promise<CheckoutSession> {
    const session = await this.request<CheckoutSession>("/checkouts", {
      method: "POST",
      body: JSON.stringify({ amount_cents: amountCents, currency, metadata, payment_methods: ["crypto"] })
    });
    return session;
  }
